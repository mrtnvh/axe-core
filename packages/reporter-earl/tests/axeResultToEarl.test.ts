...
test(`returns an array of assertions`, () => {
  ...
  // expect(dummyData.violations.length).not.toEqual(0);
  const earl = createEarlReportFromAxeResults(dummyData);
  expect(earl).toMatchSnapshot();
...
// test.skip(`Assertions have {'assertedBy': Object }`, async () => {
...
//         const result: axe.NodeResult[] = (ruleData as any)[type];
...
//         expect(axe.Result.length).toEqual(ruleAsserts.length);

//         const expectAssert = testResult(ruleData, axe.Result);
